...
Ramos's team and are quoted case by case.

## Volume discount tiers

Applied to the combined monthly last-mile and returns processing subtotal, based
on average monthly order volume:

- Up to 999 orders per month: no discount.
- 1,000 to 4,999 orders per month: 5% discount tier.
- 5,000 to 19,999 orders per month: 8% discount tier.
...
Storage is priced per cubic meter, but a client's request usually states monthly
order volume instead. For proposal estimates, TrackFlow assumes an average of
0.012 cubic meters of occupied storage per monthly order.

This is a planning assumption, not a measurement. Any proposal that uses it must
state it explicitly, and the storage line must be labelled as an estimate to be
confirmed once real dimensions are known.

## Returns rate assumption

Where a client does not state an expected return rate, proposals assume 6% of
monthly orders are returned. This assumption must also be stated wherever it is
used.

...
runner, Prefect-as-library extract/transform/load flow, and optional private R2 transform cache.
...
Prefect writes only beneath the `/tmp` tmpfs and all Prefect analytics/telemetry settings are
disabled. A completely absent R2 configuration disables cache reuse without disabling reports.

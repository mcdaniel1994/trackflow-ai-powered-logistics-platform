...
export function MobileNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 grid grid-cols-5 border-t border-mist bg-white md:hidden"
    ...
    >
      {navItems.map((item) => (
        <Link
          key={item.href}
          href={item.href}
          className="px-1 py-3 text-center text-xs font-bold text-neutral-600 transition hover:bg-ivory hover:text-navy"
        >
          {item.label}
        </Link>
      ))}

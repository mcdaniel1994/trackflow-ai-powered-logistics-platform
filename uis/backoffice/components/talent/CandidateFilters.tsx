...
import { isStage, isStatus, stageLabel, stageOptions, statusLabel, statusOptions } from "@/lib/talent/labels";
...
export function CandidateFilters({ initialStatus, initialStage, initialQ }: CandidateFiltersProps) {
  ...
  const replaceParam = useCallback(
    (name: string, value: string) => {
      ...
      // Changing any filter invalidates the current page position — a page-3
      // cursor into a differently-filtered list is meaningless. Dropping the
      // param resets the table to page 1.
      params.delete("page");
  ...
  // (e.g. back/forward navigation, the "Clear" button below). State is adjusted
  // during render rather than in an effect — React re-renders immediately with
  // the new value instead of paint-then-rerender.
  ...
  }, [currentQ, searchValue, replaceParam]);
